...
- 后续迁移必须先按风险分层处理：
  ...
  - 2026-05-18 product-domain readiness update: MiniApp draft DTO/response shape,
    draft/customization storage path shape, runtime executable search-plan helpers,
    customization draft-apply / built-in update-decline metadata policy, and
    Git function-agent diff truncation policy now live in
    `bitfun-product-domains`. Core still owns MiniApp filesystem IO, worker
    process execution, built-in asset seeding/source-hash lookup,
    host-dispatch execution, `PathManager` integration, function-agent Git/AI
    calls, prompt templates, JSON extraction, and error mapping.
