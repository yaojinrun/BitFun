...
- 2026-05-18 update: MiniApp draft manifest/response DTO, draft/customization storage path helpers, runtime executable search-plan helpers, customization draft-apply metadata policy, and built-in update/decline metadata decisions have been moved to `bitfun-product-domains::miniapp`; core continues to own draft filesystem IO, built-in asset seeding/source-hash lookup, host dispatch execution, `PathManager` integration, worker process execution, and compatibility facades.
...
- 2026-05-18 update: Git function-agent diff truncation is now an owner-crate helper used by core; AI client calls, prompt template ownership, JSON extraction, error mapping, and runtime analysis execution remain core-owned.
